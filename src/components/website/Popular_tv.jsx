...
const Popular_tv = () => {
  ...
  const settings = {
    infinite: true,
    speed: 500,
    slidesToShow: width < 992 ? (width < 769 ? 2 : 3) : 5,
    slidesToScroll: 3,
    nextArrow: <NextArrow />,
    prevArrow: <PrevArrow />,
    autoplay: true,
    ...
    <div className=" p-2  w-full pt-12 ">
      ...
      <div className="w-full p-4  ">
        <Slider focusOnSelect {...settings}>
          {popular_tv.map((movie) => (
            <Cards type={true} key={movie.id} movie={movie} />
          ))}
        </Slider>
      </div>
    </div>
